...
const checkApprovedEmail = async(email)=>{
  ...
  .get()
  .then(querySnapshot => {
    let result = querySnapshot.docs.filter(doc => doc.email ===email)
    return result;
  });
  if(res.length ===0)
     return false;
...
function* registerWithEmailPassword({ payload }) {
  ...
  const { history } = payload;
   try {
    ...
    {
      console.log("error");
      yield put(registerUserError("Sorry, we can't register your email. Please contact nexus lab management for support"));
    }
